class UsersController < ApplicationController
  ...
  def last_video
    ...
    @video = wideo.attributes.except("created_at", "updated_at").merge!(:genre_name => wideo.genres.pluck(:name).join(', '), last_video: status, :starred_status => @user.my_favorites.in? wideo)
    if @video
      render :json => {
                      :response_code => 200,
                      :response_message => "Genres has been successfully fetched.",
